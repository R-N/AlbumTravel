...
class Paket_travel_model extends General_model {
    ...
    public function count_paket_travel_customer($search=null){
        ...
        $sql = '
            SELECT 
                count(*) AS count FROM TRAVEL T, 
                PAKET_TRAVEL PT, 
                ANGGOTA_GRUP AG, 
                CUSTOMER C 
            WHERE 
                T.id_travel=PT.id_travel 
        ...
        if($search != null && $search != ''){
            $sql = $sql . ' 
                AND (
                    PT.id_paket_travel=? 
                    OR PT.nama_paket_travel LIKE "%?%" 
                    OR DATE_FORMAT(
                        PT.tanggal_keberangkatan, 
                        "%Y-%m-%d"
                    ) LIKE "%?%" 
                    OR T.nama_travel LIKE "%?%"
                )
            ';
    ...
    public function _fetch_paket_travel_customer($limit=10, $offset=0, $search=null){
        ...
        $sql = '
            SELECT 
                ...
                PT.tanggal_keberangkatan, 
                PT.ringkasan_paket_travel, 
                lama_keberangkatan, 
                harga_paket_travel, 
                AG.status_anggota_grup 
            FROM 
                TRAVEL T, 
                PAKET_TRAVEL PT, 
                ANGGOTA_GRUP AG, 
        ...
        $sql = $sql . ' 
            ORDER BY 
                AG.status_anggota_grup ASC, 
                PT.id_paket_travel DESC
        ...
        $sql = '
            ...
            FROM 
                ...
                LEFT JOIN (
                    SELECT 
                        AG0.id_anggota_grup, 
                        AG0.status_anggota_grup, 
                        AG0.id_paket_travel 
                    FROM 
                        ANGGOTA_GRUP AG0, 
                        CUSTOMER C 
                    WHERE 
                        AG0.id_customer=C.id_customer 
                        AND C.id_pengguna=?
                ) AG 
                    ON AG.id_paket_travel=PT.id_paket_travel 
            WHERE 
                T.id_travel=? 
                AND T.id_travel=PT.id_travel
        ';
    ...
    public function _fetch_anggota_grup($id_paket_travel, $limit=10, $offset=0, $search=null){
        ...
        $sql = '
            ...
            WHERE 
                AG.id_paket_travel=? 
                AND C.id_customer=AG.id_customer
        ';

        if($this->cek_peran_pengguna(2)){
            $sql = $sql . ' 
                AND AG.id_paket_travel IN (
                    SELECT AG2.id_paket_travel 
                    FROM 
                        ANGGOTA_GRUP AG2, 
                        PAKET_TRAVEL PT, 
                        TRAVEL T 
                    WHERE 
                        AG2.id_paket_travel=? 
                        AND AG2.id_paket_travel=PT.id_paket_travel 
            ...
            $sql = $sql . ' 
                AND AG.id_customer IN (
                    ...
                    WHERE 
                        AG2.id_paket_travel=? 
                        AND AG2.id_customer=C.id_customer 
                        AND C.id_pengguna=?
                )
            ';
        }
        ...
        $sql = '
            ...
            WHERE 
                ...
                AND AG.id_customer IN (
                    ...
                    WHERE 
                        AG2.id_anggota_grup=? 
                        AND AG2.id_paket_travel=PT2.id_paket_travel 
                        AND PT2.id_travel=T2.id_travel 
                        AND T2.id_pengguna=?
                )
        ';
    ...
    public function terima_anggota_grup($id_anggota_grup){
        $id_pengguna = $this->get_id_pengguna();
        $sql = '
            UPDATE ANGGOTA_GRUP AG 
            SET AG.status_anggota_grup=1 
            WHERE 
                AG.id_anggota_grup=? 
                AND AG.status_anggota_grup=0 
                AND AG.id_anggota_grup IN (
                    SELECT AG2.id_anggota_grup 
                    FROM 
                        (
                            SELECT * 
                            FROM ANGGOTA_GRUP
    ...
    public function hapus_anggota_grup($id_anggota_grup){
        $id_pengguna = $this->get_id_pengguna();
        $sql = '
            DELETE FROM ANGGOTA_GRUP 
            WHERE 
                id_anggota_grup=? 
                AND id_anggota_grup IN (
                    SELECT AG2.id_anggota_grup 
                    FROM 
                        (
                            SELECT 
                                AG3.id_anggota_grup, 
                                AG3.id_paket_travel 
                            FROM ANGGOTA_GRUP AG3 
                            WHERE AG3.id_anggota_grup=?
                        ) AG2, 
                        PAKET_TRAVEL PT, 
                        TRAVEL T 
                    WHERE 
                        AG2.id_paket_travel=PT.id_paket_travel 
                        AND PT.id_travel=T.id_travel 
                        AND T.id_pengguna=?
                )
        ';
    ...
    function tambah_album_grup($id_paket_travel, $judul_album){
        ...
        $sql = "
            INSERT INTO 
            ALBUM_GRUP(id_album, id_paket_travel) 
            VALUES(?,?)
        ";
        $query = $this->db->query($sql, array(
            $id_album,
