...
class Anggota_grup_model extends General_model {
	...
	function get_id_anggota($id_paket_travel){
		$sql = "SELECT AG.id_anggota_grup FROM PAKET_TRAVEL PT, ANGGOTA_GRUP AG, CUSTOMER C WHERE PT.id_paket_travel=? AND PT.id_paket_travel=AG.id_paket_travel AND AG.id_customer=C.id_customer AND C.id_pengguna=?";
	...
	public function join_grup($id_paket_travel){
		$sql = "INSERT INTO ANGGOTA_GRUP(id_customer, id_paket_travel) SELECT C.id_customer, ? FROM CUSTOMER C WHERE C.id_pengguna=?";
		$query = $this->db->query($sql, array(
			$id_paket_travel,
			$this->get_id_pengguna()
		));
	...
	function tambah_album_anggota($id_paket_travel, $judul_album){
		...
		$sql = "INSERT INTO ALBUM_ANGGOTA(id_album, id_anggota) VALUES(?,?)";
		$query = $this->db->query($sql, array(
			$id_album,
			$id_anggota_grup
		));
