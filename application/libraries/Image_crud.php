...
class Image_crud {
	...
	protected $table_name = 'FOTO';
	protected $priority_field = 'prioritas_foto';
	protected $url_field = 'url_foto';
	protected $title_field = 'judul_foto';
	protected $subject = 'Record';
	protected $primary_key = 'id_foto';
	protected $id_paket_travel = 0;
	protected $ci = null;
	...
	public function get_add_uri($rsegments_array){
		/* Added part */
	...
	function init($id_paket_travel){
		$this->id_paket_travel = $id_paket_travel;
		$id = 0;
		if($this->get_peran_pengguna() == 1){
			$id = $this->get_id_pengguna();
		}
		$path1 = IMAGE_PATH . '/' . $id_paket_travel;
		$path2 = $path1 . '/' . $id;
		
		$apath1 = './' . $path1;
		$apath2 = './' . $path2;
		
		if(!is_dir($apath1)) mkdir($apath1);
		if(!is_dir($apath2)) mkdir($apath2);
		
		$this->image_path = $path2;
	}
	...
	public function _library_view($view, $vars = array(), $return = FALSE)
	{
	...
	protected function get_id_pengguna(){
		return $this->ci->session->userdata['session']['id_pengguna'];
	}
	
	protected function get_peran_pengguna(){
		return $this->ci->session->userdata['session']['peran_pengguna'];
	}

    protected function _insert_table($url)
    {
		...
		$sql = 'INSERT INTO FOTO(url_foto, judul_foto) VALUES(?, ?)';
		...
		try{
			...
			if($peran_pengguna == 2 || $peran_pengguna==4){
				$sql = 'INSERT INTO FOTO_GRUP(id_foto, id_paket_travel) VALUES(?, ?)';
				
				$sql2 = 'SELECT PT.id_paket_travel AS rel_id FROM PAKET_TRAVEL PT, TRAVEL T WHERE PT.id_paket_travel=? AND PT.id_travel=T.id_travel AND T.id_pengguna=?';
				
				$query = $this->ci->db->query($sql2, array(
					$this->id_paket_travel,
					$id_pengguna
				));
				$result = $query->row();
				...
				$data = array(
					'id_foto' => $id,
					'id_paket_travel' => $rel_id
				);
				
				$this->ci->db->insert('FOTO_GRUP', $data);
			}else if($peran_pengguna == 1){
				$sql = 'INSERT INTO FOTO_ANGGOTA(id_foto, id_anggota_grup) VALUES(?, ?)';
				
				$sql2 = 'SELECT AG.id_anggota_grup AS rel_id FROM CUSTOMER C, ANGGOTA_GRUP AG, PAKET_TRAVEL PT WHERE PT.id_paket_travel=? AND PT.id_paket_travel=AG.id_paket_travel AND AG.id_customer=C.id_customer AND C.id_pengguna=?';
				...
				$data = array(
					'id_foto' => $id,
					'id_anggota_grup' => $rel_id
				);
				
				$this->ci->db->insert('FOTO_ANGGOTA', $data);
			}else{
    ...
    protected function _delete_file($id)
    {
		...
		if($peran_pengguna == 2 || $peran_pengguna == 4){
			$table2 = 'FOTO_GRUP';
			$sql = 'SELECT F.id_foto, F.url_foto FROM FOTO F, FOTO_GRUP FG, PAKET_TRAVEL PT, TRAVEL T WHERE F.id_foto=? AND F.id_foto=FG.id_foto AND FG.id_paket_travel=PT.id_paket_travel AND PT.id_travel=T.id_travel AND T.id_pengguna=?';
		}else if ($peran_pengguna==1){
			$table2 = 'FOTO_ANGGOTA';
			$sql = 'SELECT F.id_foto, F.url_foto FROM FOTO F, FOTO_ANGGOTA FA, CUSTOMER C, ANGGOTA_GRUP AG, PAKET_TRAVEL PT WHERE F.id_foto=? AND F.id_foto=FA.id_foto AND FA.id_anggota_grup=AG.id_anggota_grup AND AG.id_customer=C.id_customer AND C.id_pengguna=?';;
		}
		$result = $this->ci->db->query($sql, array(
			$id,
			$id_pengguna
		))->row();
		...
		if($result != null){
			...
			unlink( $this->get_thumbnail_url($result->{$this->url_field}) );
			unlink( $result->{$this->url_field} );

			$this->ci->db->trans_start();
			$this->ci->db->delete($table2, array($this->primary_key => $id));
    ...
    protected function _get_delete_url($value)
    {
    	...
    	return $this->site_url($ruri.'/delete_file/'.$value);
    }
    ...
    protected function _get_photos($relation_value = null)
    {
		...
		$sql = '';
		if($peran_pengguna == 2 || $peran_pengguna == 4){
			$sql = 'SELECT F.id_foto, F.url_foto, F.judul_foto FROM FOTO F, FOTO_GRUP FG, PAKET_TRAVEL PT, TRAVEL T WHERE F.id_foto=FG.id_foto AND FG.id_paket_travel=PT.id_paket_travel AND PT.id_paket_travel=? AND PT.id_travel=T.id_travel AND T.id_pengguna=?';
		}else if ($peran_pengguna==1){
			$sql = 'SELECT F.id_foto, F.url_foto, F.judul_foto FROM FOTO F, FOTO_ANGGOTA FA, CUSTOMER C, ANGGOTA_GRUP AG, PAKET_TRAVEL PT WHERE F.id_foto=FA.id_foto AND FA.id_anggota_grup=AG.id_anggota_grup AND AG.id_paket_travel=? AND AG.id_customer=C.id_customer AND C.id_pengguna=?';;
		}
		
    	$results = $this->ci->db->query($sql, array(
			$this->id_paket_travel,
    	...
    	foreach($results as $num => $row)
    	{
    		...
    		$results[$num]->image_url = $this->getFullUrl().'/'.$row->{$this->url_field};
    		$results[$num]->thumbnail_url = $this->get_thumbnail_url($results[$num]->image_url);
	...
	function get_thumbnail_url($image_url){
		$splitted = explode('/', $image_url);
		$last = count($splitted)-1;
		$splitted[$last] = $this->thumbnail_prefix.$splitted[$last];
	...
	public function getState()
	{
		...
		if(!isset($rsegments_array[4]))
		{
			$upload_url = $this->site_url($ruri.'/upload_file/');
			$ajax_list_url  = $this->site_url($ruri.'/ajax_list');
			$ordering_url  = $this->site_url($ruri.'/ordering');
			$insert_title_url  = $this->site_url($ruri.'/insert_title');
		...
		{
			$state = array( 'name' => 'delete_file', 'id' => $rsegments_array[5]);
			return (object)$state;
		}
			...
			switch ($state_info->name) {
				...
				case 'upload_file':
					...
					if ($file_name !== false) {
						...
						$this->_insert_table($this->image_path . '/' . $file_name, $state_info->relation_value);
...
class ImageUploadHandler
{
	...
	function __construct($options=null) {
		...
		*/
		...
		if ($options) {
			// Or else for PHP >= 5.3.0 use: $this->options = array_replace_recursive($this->options, $options);a
