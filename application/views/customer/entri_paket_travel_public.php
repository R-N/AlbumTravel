	...
	<div class="card" >
		...
		<div class="card-body">
		  ...
		  <p class="card-text"><?=$ringkasan_paket_travel?></p>
			<?php if($status_anggota_grup ==null){?>
				<a class="btn btn-primary text-white" onclick="join_grup(<?=$id_paket_travel?>)">Gabung</a>
			<?php }else if ($status_anggota_grup==0){ ?>
				<p class="card-text alert alert-warning">Menunggu Persetujuan</p>
			<?php }else if ($status_anggota_grup==1){ ?>
				<p class="card-text alert alert-success">Sudah bergabung</p>
			<?php } ?>
		</div>
	</div>
  </div>